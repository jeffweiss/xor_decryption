...
valid_chars_list(true) ->
    valid_chars_list(false) ++ "/;:|`~~@\\#$%^&*_=+{}<>[]";
valid_chars_list(false) ->
    lists:seq($a,$z) ++ lists:seq($A, $Z) ++ lists:seq($0, $9) ++ " !.?,'\"-()\t\r\n".
    

valid_chars(Bool) ->
    sets:from_list(valid_chars_list(Bool)).
    
keyspace() ->
    Letters = lists:seq($a, $z),
    [ [A] ++ [B] ++ [C] ++ [D] || A <- Letters, B <- Letters, C <- Letters, D <- Letters].

process(IntArray, Key, Valid) ->
    processrec(IntArray, Key, Key, Valid, 3, 0, []).

processrec(_, _, OrigKey, _, AllowableBad, AccumulatedBad, _) when AllowableBad =< AccumulatedBad ->
        {OrigKey, bad};
processrec([I|Is], [K|Ks], OrigKey, Valid, AllowableBad, AccumulatedBad, AcculatedResults) ->
    CipherChar = I bxor K,
    case sets:is_element(CipherChar, Valid) of
        true ->
            processrec(Is, Ks ++ [K], OrigKey, Valid, AllowableBad, AccumulatedBad, AcculatedResults ++ [CipherChar]);
        false ->
            processrec(Is, Ks ++ [K], OrigKey, Valid, AllowableBad, AccumulatedBad + 1, AcculatedResults  ++ [CipherChar])
    end;
processrec([], _Key, OrigKey, _Valid, _AllowableBad, _AccumulatedBad, AcculatedResults) ->
    {OrigKey, AcculatedResults}.

display({Key, Plaintext}) ->
    io:format("CIPHER: ~s~nPLAINTEXT:~n~s", [Key, Plaintext]).
...
bruteforce(Cipher) ->
    Valid = valid_chars(),
    lists:foreach(fun(X)->display(X) end, lists:filter(fun({_,X}) -> not(X == bad) end, [ process(Cipher, K, Valid) || K <- keyspace() ])).
