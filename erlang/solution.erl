...
process(IntArray, Key) ->
    processrec(IntArray, Key, valid_chars(), 3, 0, []).

processrec(_, _, _, AllowableBad, AccumulatedBad, _) when AllowableBad =< AccumulatedBad ->
        bad;
processrec([I|Is], [K|Ks], Valid, AllowableBad, AccumulatedBad, AcculatedResults) ->
    CipherChar = I bxor K,
    case sets:is_element(CipherChar, Valid) of
        true ->
            processrec(Is, Ks ++ [K], Valid, AllowableBad, AccumulatedBad, AcculatedResults ++ [CipherChar]);
        false ->
            processrec(Is, Ks ++ [K], Valid, AllowableBad, AccumulatedBad + 1, AcculatedResults  ++ [CipherChar])
    end;
processrec([], _Key, _Valid, _AllowableBad, _AccumulatedBad, AcculatedResults) ->
    AcculatedResults.

bruteforce(Cipher) ->
    lists:filter(fun(X) -> not(X == bad) end, [ process(Cipher, K) || K <- keyspace() ]).
